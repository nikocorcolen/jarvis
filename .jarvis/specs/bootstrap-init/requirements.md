...
`jarvis init` is the entry point to a Jarvis project. It creates the
`.jarvis/` directory with steering templates and configuration, detects
...
code. Without this command, no other Jarvis command has a place to write.
...
**I want to** run `jarvis init` and get a working `.jarvis/` directory
**So that** I can immediately start writing my first spec

**Acceptance criteria:**
- WHEN the user runs `jarvis init` in a directory without `.jarvis/`
  THE SYSTEM SHALL create `.jarvis/` with subdirectories `steering/`
  and `specs/`
...
- WHEN initialization completes THE SYSTEM SHALL write a
  `.jarvis/config.json` file with `formatVersion`, `createdAt` and
  `createdBy` fields
...
**As a** developer adopting Jarvis on an existing project
**I want to** run `jarvis init` and have it pre-fill what it can detect
**So that** I do not have to type information that is already in my
package.json or equivalent

**Acceptance criteria:**
- WHEN the user runs `jarvis init` and the current directory contains
...
**As a** developer who already has a Jarvis project
**I want to** be protected from accidentally re-initializing
**So that** I do not lose existing steering or specs

**Acceptance criteria:**
- WHEN the user runs `jarvis init` and `.jarvis/` already exists at the
...
- WHEN refusing to overwrite THE SYSTEM SHALL print a clear message
  pointing to the existing `.jarvis/` path and suggesting the user
...
- **NFR-001**: `jarvis init` SHALL complete in under 500 ms on a
...
- Initializing inside a subdirectory of an existing Jarvis project.

## 6. Open questions

- [x] Should `init` work from a subdirectory? → No. It writes `.jarvis/`
