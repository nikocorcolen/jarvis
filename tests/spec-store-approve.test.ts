...
async function setup() {
  const root = await mkdtemp(join(tmpdir(), 'jarvis-approve-'));
...
describe('approvePhase (T-004, US-002, FR-001)', () => {
  it('approves requirements first time and refreshes updated', async () => {
    const f = await setup();
    try {
      await createSpec({ jarvisDir: f.jarvisDir, name: 'x', today: () => D1 });
      ...
      const state = await readSpecState(f.jarvisDir, 'x');
  ...
  it('returns phase-file-missing when the file was deleted', async () => {
    ...
    try {
      ...
      const tasksPath = join(f.jarvisDir, 'specs', 'x', 'tasks.md');
      await rm(tasksPath);

      // Approve requirements + design first to avoid out-of-order short-circuit.
      await approvePhase({
        jarvisDir: f.jarvisDir, name: 'x', phase: 'requirements', today: () => D2,
      });
      await approvePhase({
        jarvisDir: f.jarvisDir, name: 'x', phase: 'design', today: () => D2,
      });

      const result = await approvePhase({
        jarvisDir: f.jarvisDir, name: 'x', phase: 'tasks', today: () => D2,
      });
  ...
  it('preserves the body byte-for-byte except for the front-matter mutation', async () => {
    ...
    try {
      await createSpec({ jarvisDir: f.jarvisDir, name: 'x', today: () => D1 });
      const filePath = join(f.jarvisDir, 'specs', 'x', 'requirements.md');
